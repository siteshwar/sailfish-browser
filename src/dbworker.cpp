...
void DBWorker::migrateHistory() {
    ...
    if(execute(history_table_exists)) {
        if (history_table_exists.first()) {
            ...
            QSqlQuery update_browser_history = prepare("INSERT INTO browser_history (url, title, thumb_path, date) select "\
                                               "link.url, link.title, link.thumb_path, history.date from link, history where "\
