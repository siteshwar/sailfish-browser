...
QHash<int, QByteArray> DeclarativePrivateTabModel::roleNames() const
{
...
void DeclarativePrivateTabModel::addTab(const QString& url, const QString &title) {
...
int DeclarativePrivateTabModel::nextTabId() const
{
    return m_nextTabId;
}

void DeclarativePrivateTabModel::remove(int index) {
...
void DeclarativePrivateTabModel::removeTabById(int tabId, bool activeTab)
{
    int index = -1;
    if (!activeTab) {
...
void DeclarativePrivateTabModel::clear()
{
    if (count() == 0)
        return;
...
bool DeclarativePrivateTabModel::activateTab(const QString& url)
{
...
bool DeclarativePrivateTabModel::activateTab(int index, bool loadActiveTab)
{
...
bool DeclarativePrivateTabModel::activateTabById(int tabId)
{
...
/**
 * @brief DeclarativePrivateTabModel::closeActiveTab
 ...
 */
void DeclarativePrivateTabModel::closeActiveTab()
{
    if (!m_tabs.isEmpty()) {
...
void DeclarativePrivateTabModel::newTab(const QString &url, const QString &title, int parentId)
{
    setWaitingForNewTab(true);
    emit newTabRequested(url, title, parentId);
}

void DeclarativePrivateTabModel::dumpTabs() const
{
    for (int i = 0; i < m_tabs.size(); i++) {
...
int DeclarativePrivateTabModel::activeTabIndex() const
{
    return findTabIndex(m_activeTab.tabId());
}

int DeclarativePrivateTabModel::count() const
{
    return m_tabs.count();
}

int DeclarativePrivateTabModel::rowCount(const QModelIndex & parent) const {
    Q_UNUSED(parent);
    return m_tabs.count();
}

QVariant DeclarativePrivateTabModel::data(const QModelIndex & index, int role) const {
...
void DeclarativePrivateTabModel::componentComplete()
{
}

bool DeclarativePrivateTabModel::loaded() const
{
    return m_loaded;
}

void DeclarativePrivateTabModel::setUnloaded()
{
    if (m_loaded) {
        m_loaded = false;
        emit loadedChanged();
...
bool DeclarativePrivateTabModel::waitingForNewTab() const
{
    return m_waitingForNewTab;
}

void DeclarativePrivateTabModel::setWaitingForNewTab(bool waiting)
{
...
const QList<Tab> &DeclarativePrivateTabModel::tabs() const
{
    return m_tabs;
}

const Tab &DeclarativePrivateTabModel::activeTab() const
{
    return m_activeTab;
}

bool DeclarativePrivateTabModel::contains(int tabId) const
{
...
void DeclarativePrivateTabModel::updateUrl(int tabId, bool activeTab, QString url, bool backForwardNavigation, bool initialLoad)
{
...
void DeclarativePrivateTabModel::updateTitle(int tabId, bool activeTab, QString url, QString title)
{
...
int DeclarativePrivateTabModel::findTabIndex(int tabId) const
{
    for (int i = 0; i < m_tabs.size(); i++) {
...
void DeclarativePrivateTabModel::updateActiveTab(const Tab &activeTab, bool loadActiveTab)
{
#if DEBUG_LOGS
...
void DeclarativePrivateTabModel::updateTabUrl(int tabId, bool activeTab, const QString &url, bool navigate)
{
    if (!LinkValidator::navigable(QUrl(url))) {
#if DEBUG_LOGS
